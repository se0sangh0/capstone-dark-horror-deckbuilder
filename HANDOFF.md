...
> 마지막 갱신: **2026-07-29** (AI 협의체 완전 연결 — Node 22에 설치된 CommandCode 1.4.5를 `/Users/seosangho/.local/bin/cmdc` 고정 래퍼로 복구. 인증·모델 목록과 `deepseek/deepseek-v4-pro` max-effort 프로젝트 읽기 스모크 테스트 성공. Gemini 3.6 Flash와 함께 3모델 협의체 사용 가능)
> 마지막 갱신: **2026-07-29** (디자인 프롬프트·협의체 정합성 보강 — 초기 농경 신앙 6개 동료의 복사용 `FILTERED` 스프라이트/`TRUE_VIEW` 초상 프롬프트 추가, CA-02·04 역할 현행화, CA-07 자세 모순과 D6-ap DoT 이력 모순 수정. ThermoGuard식 `AGENTS.md` 이식 및 Gemini 3.6 Flash 실연결 성공)
...
## 디자인 프롬프트·AI 협의체 정합성 보강 세션 (2026-07-29)

- 기획 진행 위치는 바뀌지 않았다. 6개 동료 시각 명세와 주술사 전투 개념 명세까지 완료됐으며, 다음 기획 Q&A는 질문 48의 범위 선택부터 재개한다.
- [캐릭터 디자인 프롬프트](디자인_UI/00_캐릭터_디자인_프롬프트.md)에 신규 제작용 공통 스타일 잠금, 6개 역할별 `FILTERED` 전투 스프라이트 모듈, `TRUE_VIEW` 남녀 얼굴 초상 변환 모듈을 추가했다. 기존 서양 판타지 영문 프롬프트는 §2 보관용으로 분리하고 신규 제작 금지를 명시했다.
- [컨셉 아트 프롬프트](디자인_UI/00_컨셉_아트_GPT_프롬프트_가이드.md)의 CA-02·CA-04 동료 역할·소품을 최신 초기 농경 신앙 명세로 바꾸고, CA-07의 `stands protectively`/`kneeling guardian` 자세 모순을 `standing guardian`으로 통일했다. CA-01·03·06의 현대·타세계 재질 혼합은 오류가 아니라 다세계 봉합 키 아트용이라는 범위를 명시했다.
- [15_미결_안건표](기획_통합/15_미결_안건표.md)의 D6-ap에 남아 있던 `같은 종류 지속 피해 갱신` 문구를 D6-at 철회 이력에 맞게 정정했다.
- 루트 [AGENTS.md](AGENTS.md)에 Codex=최종 PM, DeepSeek=구현 검토, Gemini=독립 분석·QA 역할과 단일 기준·인식 레이어·안전 규칙을 이식했다.
- Gemini 3.6 Flash High는 Antigravity CLI에서 프로젝트 `AGENTS.md` 직접 읽기와 독립 문서 검토에 성공했다. 헤드리스 직접 읽기는 작업공간 연결과 권한 설정이 필요하므로 기본은 Codex가 필요한 발췌만 전달하는 읽기 전용 검토 방식으로 운용한다.
- DeepSeek 장애 원인은 삭제가 아니라 Node 런타임 불일치였다. CommandCode 1.4.5와 인증·`deepseek/deepseek-v4-pro` 설정은 Node 22 아래 정상 설치돼 있었지만 Codex 셸이 Node 20을 사용해 `cmdc`가 PATH에서 보이지 않았다. `/Users/seosangho/.local/bin/cmdc`가 Node 22 실행 파일을 고정 호출하도록 복구했다.
- `cmdc status`, `cmdc --list-models`를 통과했고 `deepseek/deepseek-v4-pro`·`--effort max`·읽기 전용 plan 모드로 현재 프로젝트 `AGENTS.md`를 직접 읽어 `DEEPSEEK_V4_PRO_OK` 응답을 확인했다. 이후 협의체에서는 모델이 불명확한 `deepseek-worker`보다 이 검증된 명시 호출을 우선한다.
- Unity 코드·씬·프리팹·에셋은 수정하지 않았으며 Unity 컴파일·플레이 검증 대상도 없다. Markdown 코드 펜스 짝, `git diff --check`, 구형 충돌 문구 재검색을 통과했다.
...
- **현재 구현 코드는 최신 기획과 차이가 크므로 코드보다 Notion의 [괴이탐사국 팀 기획서](https://app.notion.com/p/3af6aef5baac800298b6c2154b9b2699)를 기준으로 작업한다.**
- 기획자와 결정할 때는 한 번에 질문 하나만 제시하고, 2~3개 구체 선택지와 권장안을 제공한다.
- 결정된 답변은 관련 Notion 기획 문서·스토리보드와 필요한 디자인 자료에 즉시 반영하고 [15. 미결 안건표](https://app.notion.com/p/3af6aef5baac8185866ff29253c2e69f)의 완료 항목으로 남긴다.
- 현재 기획의 단일 기준은 Notion 팀 기획서이며, `기획_통합/`은 2026-08-01 이전 시점의 읽기 전용 아카이브다.
- 디자인 기준과 AI 생성 결과는 Notion의 [디자인 가이드](https://app.notion.com/p/e749c2ad0b9a417f91ec85ce1641c73a)와 [AI 디자인 생성 기록](https://app.notion.com/p/5c53f3ac5a5d47359d6c8e8c419254a1)에서 관리한다.
- 시스템 기획은 항목마다 명시적 주어와 한 가지 지시로 한 기능만 설명하고, 문서 변경 시 [변경 이력](https://app.notion.com/p/8f07bd2183f341deb8e0f676994f3961)을 함께 갱신한다.
