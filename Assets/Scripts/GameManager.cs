...
public class GameManager : Singleton<GameManager>
{
    ...
    {
        // [GameManager] 내 턴 시작 로그 — 매 턴 반복되어 주석 처리
        ...
        for (int i = 0; i < myCards.Length; i++)
        {
            ...
            // PartyManager 가 없거나 다수파 계산 실패 시에만 카드 소유자 affinity 로 폴백.
    ...
    public void OnCardUsed(StackCardController usedCard)
    {
        ...
        // 평소에는 매 턴 손패를 다 써도 사용자가 직접 [턴 종료] 를 눌러야 다음 턴으로 간다.
        // 단, 전체 덱이 모두 소진되어 다음 턴에 더 이상 카드를 뽑을 수 없는 상황에서
        // 손패까지 다 사용했다면 → 사용자가 누를 카드도 없으므로 자동으로 턴을 넘긴다.
    ...
    /// <summary>
    /// 드로우 덱이 끝까지 소진되었는지 여부.
    /// currentDrawIndex 가 drawDeck 길이 이상이면 더 뽑을 카드가 없는 상태.
    /// </summary>
    ...
    /// 빈 슬롯(SetActive(false)) 은 카드가 없는 슬롯이므로 무시.
    /// 활성 카드가 0장이면(완전 빈 손패) true 로 본다 — 덱 고갈과 같이 체크되므로 안전.
    /// </summary>
    private bool AreAllActiveCardsUsed()
    {
        ...
        foreach (var card in myCards)
        {
            ...
            if (!card.gameObject.activeSelf) continue;   // 빈 슬롯 무시
            if (!card.isUsed) return false;               // 아직 안 쓴 카드 발견
        }
        return true;
    }
