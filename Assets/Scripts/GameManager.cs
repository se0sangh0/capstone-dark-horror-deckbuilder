...
public class GameManager : MonoBehaviour
{
    ...
    // -------------------------------------------------------
    // 드로우 페이즈 — 카드 세팅
    // TODO: 성향(Affinity)은 CompanionData 에서 참조해야 함.
    // 현재 카드에 성향 필드가 제거되었으므로, 추후 BattleManager 나
    // 덱 빌딩 로직에서 성향을 주입받도록 수정 필요.
    // -------------------------------------------------------
    public void StartMyTurn()
    {
        ...
        {
            // 임시: 모든 카드를 낙천가(Optimist) 범위로 처리 (추후 수정 필요)
            int stackValue = GenerateStackValue(CardAffinity.Optimist);
