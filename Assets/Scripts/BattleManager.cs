...
public partial class BattleManager : Singleton<BattleManager>
{
    ...
    /// <summary>InitBattle 정보 로그를 첫 전투에서만 출력하기 위한 플래그 (매 전투 동일 정보 반복 방지)</summary>
    ...
    private void InitBattle()
    {
        ...
        // ── ✨ 이전 전투에서 남은 역할별 스택 리셋 (배틀 노드 클리어 후 다음 노드 누적 방지) ──
        ...
        foreach (var fellow in fellows)
        {
            ...
            // (스트레스는 기획상 화톳불 노드에서만 회복되므로 전투 진입 때 리셋 안 함)
            fellow.isDead          = false;
            ...
            // ⚠️ 매 전투 진입마다 풀 회복하면 안 됨 (이전 전투에서 남은 HP 유지가 정상)
            // → CurrentHp 가 0(미초기화) 일 때만 maxHp 로 시작. 그 외엔 유지.
